# -*- coding: utf-8 -*-
"""
UNIFIED FUTURES BOT v2.5.8 SAFE
MEXC + BITGET | 15m | объём >= 5M | кнопки BUY/EST | авто-скан | scanlog
"""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "/scanlog — включить/выключить пруф-лог сканера\n"
        "/stop — выключить авто\n\n"
        "💡 Кнопки BUY/EST будут и в авто-сигналах."
    )
    await update.effective_message.reply_text(text, parse_mode="Markdown")

async def info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        "*UNIFIED FUTURES BOT — справка*\n\n"
        "🔎 Что делает:\n"
        "• сканирует MEXC и Bitget фьючерсы\n"
        "• фильтрует по объёму ≥5M\n"
        "• проверяет RSI, EMA 50/200, всплеск объёма, S/R\n"
        "• сверяет тренд 1h и 4h\n"
        "• считает TP1/TP2, ETA, net-прибыль\n\n"
        "📋 Команды:\n"
        "/scan — ручной поиск\n"
        "/top — самые сильные\n"
        "/report — открытые этим ботом\n"
        "/history — CSV-лог\n"
        "/scanlog — включить/выключить показ шагов скана\n"
        "/stop — отключить авто\n\n"
        "⚠️ Bitget: бот пытается сам включить isolated.\n"
        "⚠️ MEXC: сейчас чаще только аналитика, ордер руками.\n"
    )
    await update.effective_message.reply_text(text, parse_mode="Markdown")
